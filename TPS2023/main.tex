...
\frame{
\frametitle{Probing the universe with coherent waves: new era of precision mapping}
\begin{itemize}
\item wave coherent effects provide access to Fresnel Scale features
  $r_F=\sqrt{\lambda L}$.
\item for $\lambda \sim 1$m, $L\sim 10^{26}$m, map universe with
  resolution of $\theta \sim 10^{-13}$: nano arcseconds.  1000x
  higher resolving power than Event Horizon telescope.
\item requires decoding of interference pattern
\end{itemize}
}
...
\begin{itemize}
\item enables using the full wave nature of light to probe matter:
