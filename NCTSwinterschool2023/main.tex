...
\item galaxy: kpc coherent field, challenging with even
...
\begin{itemize}
\item axionic baryogenesis, motivated by strong CP problem of neutron electric dipole
\item $\dot{B}=\nabla \times E+g \dot{\theta} \nabla \times B$
\item $\dot{B}_L=-g \dot{\theta} |k| B_L$,\ \ \  $\dot{B}_R=g \dot{\theta} |k| B_R$
\item $B_L=\exp\left( -g \dot{\theta} k t) B_L(t=0)$,\ \ \
    $B_R=\exp\left( g \dot{\theta} k t) B_R(t=0)$
...
\item fractional helical power spectrum $f=\frac{L^2-R^2}{L^2+R^2}=\frac{{\rm Im}Q\bar{U}}{Q^2+U^2}$
\end{itemize}
}
...
\item direct tests for helical dynamo and primordial helical fields
  (axionic)
\item opportunity for new discovery with helical tools!
\end{itemize}
}
