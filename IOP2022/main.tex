...
\author[U. Pen]{Ue-Li Pen (ASIAA and CITA) Dylan Jow (CITA) and Job
  Feldbrugge (Edinburgh)
}
...
$x(t)=-i
\sin^{-1}\left[\frac{\sqrt{E-1}\tanh(\sqrt{E}t)}{\sqrt{E}(\tanh(\sqrt{E}t)-1)}\right]$
    \item for $\lim_{E\rightarrow 1+\epsilon^2} = \sinh^{-1} [\epsilon \sinh(t)]$
    \end{itemize}
 \includegraphics[width=2.5in]{Figures/sechreal.png}
  }
...
\item $V(x)={\rm sech}^2(x)$
  \item  exactly solvable: 
$x(t)=-i \sin^{-1}\left[\frac{\sqrt{E-1}\tanh(\sqrt{E}t)}{\sqrt{E}(\tanh(\sqrt{E}t)-1)}\right]$
